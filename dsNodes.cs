//------------------------------------------------------------------------------
// <autogenerated>
//     This code was generated by a tool.
//     Runtime Version: 1.0.3705.0
//
//     Changes to this file may cause incorrect behavior and will be lost if 
//     the code is regenerated.
// </autogenerated>
//------------------------------------------------------------------------------

namespace CafeManager {
    using System;
    using System.Data;
    using System.Xml;
    using System.Runtime.Serialization;
    
    
    [Serializable()]
    [System.ComponentModel.DesignerCategoryAttribute("code")]
    [System.Diagnostics.DebuggerStepThrough()]
    [System.ComponentModel.ToolboxItem(true)]
    public class dsNodes : DataSet {
        
        private NodesDataTable tableNodes;
        
        public dsNodes() {
            this.InitClass();
            System.ComponentModel.CollectionChangeEventHandler schemaChangedHandler = new System.ComponentModel.CollectionChangeEventHandler(this.SchemaChanged);
            this.Tables.CollectionChanged += schemaChangedHandler;
            this.Relations.CollectionChanged += schemaChangedHandler;
        }
        
        protected dsNodes(SerializationInfo info, StreamingContext context) {
            string strSchema = ((string)(info.GetValue("XmlSchema", typeof(string))));
            if ((strSchema != null)) {
                DataSet ds = new DataSet();
                ds.ReadXmlSchema(new XmlTextReader(new System.IO.StringReader(strSchema)));
                if ((ds.Tables["Nodes"] != null)) {
                    this.Tables.Add(new NodesDataTable(ds.Tables["Nodes"]));
                }
                this.DataSetName = ds.DataSetName;
                this.Prefix = ds.Prefix;
                this.Namespace = ds.Namespace;
                this.Locale = ds.Locale;
                this.CaseSensitive = ds.CaseSensitive;
                this.EnforceConstraints = ds.EnforceConstraints;
                this.Merge(ds, false, System.Data.MissingSchemaAction.Add);
                this.InitVars();
            }
            else {
                this.InitClass();
            }
            this.GetSerializationData(info, context);
            System.ComponentModel.CollectionChangeEventHandler schemaChangedHandler = new System.ComponentModel.CollectionChangeEventHandler(this.SchemaChanged);
            this.Tables.CollectionChanged += schemaChangedHandler;
            this.Relations.CollectionChanged += schemaChangedHandler;
        }
        
        [System.ComponentModel.Browsable(false)]
        [System.ComponentModel.DesignerSerializationVisibilityAttribute(System.ComponentModel.DesignerSerializationVisibility.Content)]
        public NodesDataTable Nodes {
            get {
                return this.tableNodes;
            }
        }
        
        public override DataSet Clone() {
            dsNodes cln = ((dsNodes)(base.Clone()));
            cln.InitVars();
            return cln;
        }
        
        protected override bool ShouldSerializeTables() {
            return false;
        }
        
        protected override bool ShouldSerializeRelations() {
            return false;
        }
        
        protected override void ReadXmlSerializable(XmlReader reader) {
            this.Reset();
            DataSet ds = new DataSet();
            ds.ReadXml(reader);
            if ((ds.Tables["Nodes"] != null)) {
                this.Tables.Add(new NodesDataTable(ds.Tables["Nodes"]));
            }
            this.DataSetName = ds.DataSetName;
            this.Prefix = ds.Prefix;
            this.Namespace = ds.Namespace;
            this.Locale = ds.Locale;
            this.CaseSensitive = ds.CaseSensitive;
            this.EnforceConstraints = ds.EnforceConstraints;
            this.Merge(ds, false, System.Data.MissingSchemaAction.Add);
            this.InitVars();
        }
        
        protected override System.Xml.Schema.XmlSchema GetSchemaSerializable() {
            System.IO.MemoryStream stream = new System.IO.MemoryStream();
            this.WriteXmlSchema(new XmlTextWriter(stream, null));
            stream.Position = 0;
            return System.Xml.Schema.XmlSchema.Read(new XmlTextReader(stream), null);
        }
        
        internal void InitVars() {
            this.tableNodes = ((NodesDataTable)(this.Tables["Nodes"]));
            if ((this.tableNodes != null)) {
                this.tableNodes.InitVars();
            }
        }
        
        private void InitClass() {
            this.DataSetName = "dsNodes";
            this.Prefix = "";
            this.Namespace = "http://www.tempuri.org/dsNodes.xsd";
            this.Locale = new System.Globalization.CultureInfo("en-US");
            this.CaseSensitive = false;
            this.EnforceConstraints = true;
            this.tableNodes = new NodesDataTable();
            this.Tables.Add(this.tableNodes);
        }
        
        private bool ShouldSerializeNodes() {
            return false;
        }
        
        private void SchemaChanged(object sender, System.ComponentModel.CollectionChangeEventArgs e) {
            if ((e.Action == System.ComponentModel.CollectionChangeAction.Remove)) {
                this.InitVars();
            }
        }
        
        public delegate void NodesRowChangeEventHandler(object sender, NodesRowChangeEvent e);
        
        [System.Diagnostics.DebuggerStepThrough()]
        public class NodesDataTable : DataTable, System.Collections.IEnumerable {
            
            private DataColumn columnNodes;
            
            internal NodesDataTable() : 
                    base("Nodes") {
                this.InitClass();
            }
            
            internal NodesDataTable(DataTable table) : 
                    base(table.TableName) {
                if ((table.CaseSensitive != table.DataSet.CaseSensitive)) {
                    this.CaseSensitive = table.CaseSensitive;
                }
                if ((table.Locale.ToString() != table.DataSet.Locale.ToString())) {
                    this.Locale = table.Locale;
                }
                if ((table.Namespace != table.DataSet.Namespace)) {
                    this.Namespace = table.Namespace;
                }
                this.Prefix = table.Prefix;
                this.MinimumCapacity = table.MinimumCapacity;
                this.DisplayExpression = table.DisplayExpression;
            }
            
            [System.ComponentModel.Browsable(false)]
            public int Count {
                get {
                    return this.Rows.Count;
                }
            }
            
            internal DataColumn NodesColumn {
                get {
                    return this.columnNodes;
                }
            }
            
            public NodesRow this[int index] {
                get {
                    return ((NodesRow)(this.Rows[index]));
                }
            }
            
            public event NodesRowChangeEventHandler NodesRowChanged;
            
            public event NodesRowChangeEventHandler NodesRowChanging;
            
            public event NodesRowChangeEventHandler NodesRowDeleted;
            
            public event NodesRowChangeEventHandler NodesRowDeleting;
            
            public void AddNodesRow(NodesRow row) {
                this.Rows.Add(row);
            }
            
            public NodesRow AddNodesRow(int Nodes) {
                NodesRow rowNodesRow = ((NodesRow)(this.NewRow()));
                rowNodesRow.ItemArray = new object[] {
                        Nodes};
                this.Rows.Add(rowNodesRow);
                return rowNodesRow;
            }
            
            public NodesRow FindByNodes(int Nodes) {
                return ((NodesRow)(this.Rows.Find(new object[] {
                            Nodes})));
            }
            
            public System.Collections.IEnumerator GetEnumerator() {
                return this.Rows.GetEnumerator();
            }
            
            public override DataTable Clone() {
                NodesDataTable cln = ((NodesDataTable)(base.Clone()));
                cln.InitVars();
                return cln;
            }
            
            protected override DataTable CreateInstance() {
                return new NodesDataTable();
            }
            
            internal void InitVars() {
                this.columnNodes = this.Columns["Nodes"];
            }
            
            private void InitClass() {
                this.columnNodes = new DataColumn("Nodes", typeof(int), null, System.Data.MappingType.Element);
                this.Columns.Add(this.columnNodes);
                this.Constraints.Add(new UniqueConstraint("Constraint1", new DataColumn[] {
                                this.columnNodes}, true));
                this.columnNodes.AllowDBNull = false;
                this.columnNodes.Unique = true;
            }
            
            public NodesRow NewNodesRow() {
                return ((NodesRow)(this.NewRow()));
            }
            
            protected override DataRow NewRowFromBuilder(DataRowBuilder builder) {
                return new NodesRow(builder);
            }
            
            protected override System.Type GetRowType() {
                return typeof(NodesRow);
            }
            
            protected override void OnRowChanged(DataRowChangeEventArgs e) {
                base.OnRowChanged(e);
                if ((this.NodesRowChanged != null)) {
                    this.NodesRowChanged(this, new NodesRowChangeEvent(((NodesRow)(e.Row)), e.Action));
                }
            }
            
            protected override void OnRowChanging(DataRowChangeEventArgs e) {
                base.OnRowChanging(e);
                if ((this.NodesRowChanging != null)) {
                    this.NodesRowChanging(this, new NodesRowChangeEvent(((NodesRow)(e.Row)), e.Action));
                }
            }
            
            protected override void OnRowDeleted(DataRowChangeEventArgs e) {
                base.OnRowDeleted(e);
                if ((this.NodesRowDeleted != null)) {
                    this.NodesRowDeleted(this, new NodesRowChangeEvent(((NodesRow)(e.Row)), e.Action));
                }
            }
            
            protected override void OnRowDeleting(DataRowChangeEventArgs e) {
                base.OnRowDeleting(e);
                if ((this.NodesRowDeleting != null)) {
                    this.NodesRowDeleting(this, new NodesRowChangeEvent(((NodesRow)(e.Row)), e.Action));
                }
            }
            
            public void RemoveNodesRow(NodesRow row) {
                this.Rows.Remove(row);
            }
        }
        
        [System.Diagnostics.DebuggerStepThrough()]
        public class NodesRow : DataRow {
            
            private NodesDataTable tableNodes;
            
            internal NodesRow(DataRowBuilder rb) : 
                    base(rb) {
                this.tableNodes = ((NodesDataTable)(this.Table));
            }
            
            public int Nodes {
                get {
                    return ((int)(this[this.tableNodes.NodesColumn]));
                }
                set {
                    this[this.tableNodes.NodesColumn] = value;
                }
            }
        }
        
        [System.Diagnostics.DebuggerStepThrough()]
        public class NodesRowChangeEvent : EventArgs {
            
            private NodesRow eventRow;
            
            private DataRowAction eventAction;
            
            public NodesRowChangeEvent(NodesRow row, DataRowAction action) {
                this.eventRow = row;
                this.eventAction = action;
            }
            
            public NodesRow Row {
                get {
                    return this.eventRow;
                }
            }
            
            public DataRowAction Action {
                get {
                    return this.eventAction;
                }
            }
        }
    }
}
